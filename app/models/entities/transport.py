...
from app.utils import valid_amount_charged, valid_hour_format
...
class Transport(db.Model):
    ...
    transport_hour = Column(db.String(10), nullable=False)
    ...
    def is_valid(self) -> Result:
        if (
            ...
            not self.km_quantity or self.km_quantity == 0 or
        ...
        if not self.transport_hour:
            return Result(success= False,message= "Hora de transporte não informada!")

        if not valid_hour_format(self.transport_hour):
            return Result(success= False,message= "A hora do transporte está no formato inválido. O formato correto é HH:MM!")  
        ...
        if not valid_amount_charged(self.km_quantity, self.amount_charged_by_km):
    ...
    def fill_update(self, transport: TransportViewModel):
        ...
        self.km_quantity = transport.km_quantity
